...
//    Copyright:      InterSense 2016 - All rights Reserved.
//
...
float libVersion = -1;
// For UDP data, allow reception of the joystick and AUX data globally, if requested
Bool  getAllInputs[ISD_MAX_TRACKERS];
Bool  getAllAuxInputs[ISD_MAX_TRACKERS];
...
void showTrackerStats( ISD_TRACKER_HANDLE Trackers[ISD_MAX_TRACKERS], ISD_TRACKER_HANDLE currentTrackerH, WORD currentStation, WORD logType, WORD recordsToSkip )
{
	...
	ISD_STATION_HARDWARE_INFO_TYPE		stationHwInfo[ISD_MAX_STATIONS] = {0};
	char								interfaceType[10];
	...
	for(i = 0; i <= ISD_MAX_TRACKERS; i++)
	{
		...
		switch(hwInfo.Interface)
		{
			case ISD_INTERFACE_UNKNOWN:
				sprintf(interfaceType, "Unknown");
				break;
			case ISD_INTERFACE_SERIAL:
				sprintf(interfaceType, "Serial");
				break;
			case ISD_INTERFACE_USB:
				sprintf(interfaceType, "USB");
				break;
			case ISD_INTERFACE_ETHERNET_UDP:
				sprintf(interfaceType, "UDP");
				break;
			case ISD_INTERFACE_ETHERNET_TCP:
				sprintf(interfaceType, "TCP");
				break;
			case ISD_INTERFACE_IOCARD:
				sprintf(interfaceType, "I/O Card");
				break;
			case ISD_INTERFACE_PCMCIA:
				sprintf(interfaceType, "PCMCIA");
				break;
			case ISD_INTERFACE_FILE:
				sprintf(interfaceType, "File");
				break;
			case ISD_INTERFACE_PIPE:
				sprintf(interfaceType, "Pipe");
				break;
			case ISD_INTERFACE_SFACCESS:
				sprintf(interfaceType, "sfAccess");
				break;
			default:
				sprintf(interfaceType, "Unknown");
				break;
		...
		if( Tracker.TrackerType == ISD_PRECISION_SERIES )
		{
			...
			{         
				...
				if( !ISD_GetStationConfig( Trackers[i], 
					&Stations[thisStation-1], thisStation, FALSE ) ) continue;

				if( !ISD_GetStationHardwareInfo( Trackers[i], 
					&stationHwInfo[thisStation-1], thisStation ) ) continue;
			}
		}

		if( ISD_GetTrackerConfig( Trackers[i], &Tracker, FALSE ) )
		{
			printf( "[%c] Tracker %d, port %d (%s): %s\n", 
				Trackers[i] == currentTrackerH ? '*': ' ', i+1, Tracker.Port,
				interfaceType, hwInfo.Valid ? hwInfo.ModelName : "Unknown Tracker");
			...
			printf( "  St Serial   FWVer TStamp State Enh. Sens. Comp. Pred.\n" );
				...
				if( stationHwInfo[j-1].Valid) // Skip invalid stations
				{
					if( ISD_GetStationConfig( Trackers[i], &Station, j, FALSE ))
					{
						if(Station.State == 1) // Don't display stations that are not connected
						{
							...
							printf( " %-3d", j );
							printf( "%-9s%-6g%-8s%-5s%-5u%-6u%-6u%-5u\n",
								stationHwInfo[j-1].FullSerialNum, 
			 ...
			 ISD_STATION_HARDWARE_INFO_TYPE	stationHwInfo[ISD_MAX_TRACKERS][ISD_MAX_STATIONS])
{
				...
				for(j = 0; j < numStations; j++)
				{
					if( stationHwInfo[i][j].Valid == 1)
					{
						if( ISD_GetStationConfig( Trackers[i], &Station, j+1, FALSE ))
						{
							...
							{
								fprintf(fp,"%u,%u,%s,%g,%u,%s,%s,%u,%s,%s,%u,%u,%u,%u\n",
									i+1, j+1,
									stationHwInfo[i][j].Valid ? (&stationHwInfo[i][j])->FullSerialNum : "",
	...
	if(staInfo->GetInputs || getAllInputs[trackerNum])
	{
	...
	if(staInfo->GetAuxInputs || getAllAuxInputs[trackerNum])
	{
...
{
	DWORD i;
	DWORD buttonsToShow = 0;
	DWORD auxBytesToShow = 0;
	DWORD joystickAxesToShow = 0;
	...
	if( ISD_GetCommInfo( currentTrackerH, Tracker ) )
	{
		...
		if( Tracker->TrackerModel == ISD_IS600 || 
			...
			Tracker->TrackerModel == ISD_IS1500 )
		{
		...
		if( Station->GetAuxInputs || getAllAuxInputs[currentTrackerH - 1] ) 
		{
			if(!getAllAuxInputs[currentTrackerH - 1])
			{
				auxBytesToShow = stationHwInfo->Capability.AuxInputs;
			}
			else
			{
				auxBytesToShow = 4;
		...
		if( Station->GetInputs || getAllInputs[currentTrackerH - 1] ) 
		{
			if(!getAllInputs[currentTrackerH - 1])
			{
				buttonsToShow = stationHwInfo->Capability.NumButtons;
				joystickAxesToShow = stationHwInfo->Capability.NumChannels;
			}
			else
			{
				// For UDP, assume up to 6 buttons and 2 joystick axes
				buttonsToShow = 6;
				joystickAxesToShow = 2;
			}

			// Currently available products have at most 6 buttons
			printf( "JOY:" );
			
			for( i=0; i < buttonsToShow; i++ )
			{
				printf( "%d", data->ButtonState[i] );
			}

			for( i=0; i < joystickAxesToShow; i++ )
			{
...
int main(void)
{
	...
	ISD_STATION_INFO_TYPE           Stations[ISD_MAX_TRACKERS][ISD_MAX_STATIONS];
	...
	else
	{
		...
		for( i=0; i < numOpenTrackers; i++ )
		{
			...
			configureStations(Trackers[i], Stations[i], validStation);

			// Reset the global input/AUX input flags
			getAllInputs[i] = FALSE;
			getAllAuxInputs[i] = FALSE;

			// Get hardware information for each station on each tracker; this is used in later functions
			for( j=0; j < ISD_MAX_STATIONS; j++)
				ISD_GetStationHardwareInfo(i, &StationsHwInfo[i][j], j+1);
		}
		...
		selectedStation = 1;
		// Move the selected station to the first valid station (of the initial tracker)
		for( i=1; i <= ISD_MAX_STATIONS; i++ )
		{
			if(validStation[0][i-1])
			{
				selectedStation = i;
				break;
			}
		...
		showTrackerStats( Trackers, currentTrackerH, selectedStation, logType, numRecordsToSkip );
		...
		while( !done )
		{
			...
			if( ch_key != FALSE )
			{
				...
				case ',':
					{
						BYTE AuxOutput[4];
						char buf[1024];
						int numOutputs = StationsHwInfo[currentTrackerH-1][selectedStation-1].Capability.AuxOutputs;
						...
						else
						{
							...
							ISD_AuxOutput( currentTrackerH, selectedStation, AuxOutput, numOutputs );
						}
					}
					break;
				...
				case '<':
					...
					printf( "                                                                              \r" );
					fflush(0);
					ISD_GetStationConfig( currentTrackerH, &Stations[selectedTracker-1][selectedStation-1], selectedStation, TRUE );
					Stations[selectedTracker-1][selectedStation-1].GetAuxInputs = !Stations[selectedTracker-1][selectedStation-1].GetAuxInputs;
					ISD_SetStationConfig( currentTrackerH, &Stations[selectedTracker-1][selectedStation-1], selectedStation, TRUE );

					// Handle UDP as a global flag
					if(hwInfo.Interface == ISD_INTERFACE_ETHERNET_UDP)
						getAllAuxInputs[selectedTracker - 1] = !getAllAuxInputs[selectedTracker - 1];
					break;
				...
				case '>':
					...
					fflush(0);
					ISD_GetStationConfig( currentTrackerH, &Stations[selectedTracker-1][selectedStation-1], selectedStation, TRUE );
					Stations[selectedTracker-1][selectedStation-1].GetInputs = !Stations[selectedTracker-1][selectedStation-1].GetInputs;
					ISD_SetStationConfig( currentTrackerH, &Stations[selectedTracker-1][selectedStation-1], selectedStation, TRUE );					
					
					// Handle UDP as a global flag
					if(hwInfo.Interface == ISD_INTERFACE_ETHERNET_UDP)
						getAllInputs[selectedTracker - 1] = !getAllInputs[selectedTracker - 1];
					break;
				...
				case 'e': // IS-x products only, not for InterTrax 
					...
					if( ISD_GetStationConfig( currentTrackerH,
						&Stations[selectedTracker-1][selectedStation-1], selectedStation, TRUE ) )
					{
						Stations[selectedTracker-1][selectedStation-1].Enhancement = (Stations[selectedTracker-1][selectedStation-1].Enhancement+1) % 3;
				...
				case 't':
					...
					if( ISD_GetStationConfig( currentTrackerH,
						&Stations[selectedTracker-1][selectedStation-1], selectedStation, TRUE ) )
					{
						Stations[selectedTracker-1][selectedStation-1].TimeStamped = !(Stations[selectedTracker-1][selectedStation-1].TimeStamped);
				...
				case 'c': // IS-x products only, not for InterTrax
					...
					if( ISD_GetStationConfig( currentTrackerH,
						&Stations[selectedTracker-1][selectedStation-1], selectedStation, TRUE ))
					{
						Stations[selectedTracker-1][selectedStation-1].Compass = (Stations[selectedTracker-1][selectedStation-1].Compass + 1) % 3;
				...
				case 'p': // IS-x products only, not for InterTrax 
					...
					if( ISD_GetStationConfig( currentTrackerH, 
						&Stations[selectedTracker-1][selectedStation-1], selectedStation, TRUE ))
					{
						Stations[selectedTracker-1][selectedStation-1].Prediction = (Stations[selectedTracker-1][selectedStation-1].Prediction + 10) % 60;
				...
				case 's': // IS-x products only, not for InterTrax 
					...
					if( ISD_GetStationConfig( currentTrackerH, 
						&Stations[selectedTracker-1][selectedStation-1], selectedStation, TRUE ))
					{
						Stations[selectedTracker-1][selectedStation-1].Sensitivity = (Stations[selectedTracker-1][selectedStation-1].Sensitivity + 1) % 5;

						if( Stations[selectedTracker-1][selectedStation-1].Sensitivity == 0)
							Stations[selectedTracker-1][selectedStation-1].Sensitivity = 1;
				...
				case 'n': // Cycle the currently selected tracker
					...
					selectedTracker = trackerIdx + 1;
					
					selectedStation = 1;
					// Move the selected station to the first valid station on tracker switches
					for( i=1; i <= ISD_MAX_STATIONS; i++ )
					{
						if(validStation[selectedTracker-1][selectedStation-1])
						{
							selectedStation = i;
							break;
						}
					...
					configureStations(currentTrackerH, Stations[selectedTracker-1], validStation);

					showTrackerStats( Trackers, currentTrackerH, selectedStation, logType, numRecordsToSkip );
					break;

				case 'r':
					if (ISD_ResetHeading(currentTrackerH, selectedStation))
						printf("\nHeading Reset!\n");
					break;

				case 'b':
					if (ISD_Boresight(currentTrackerH, selectedStation, TRUE))
						printf("\nBoresight set!\n");
					break;

				case 'u':
					if (ISD_Boresight(currentTrackerH, selectedStation, FALSE))
						printf("\nBoresight reset!\n");
					break;
				...
				case '1': // IS-x products only, not for InterTrax 
					...
					if( ISD_GetStationConfig( currentTrackerH,
						&Stations[selectedTracker-1][selectedStation-1], selectedStation, FALSE ) )
					{
						Stations[selectedTracker-1][selectedStation-1].GetExtendedData = TRUE;
				...
				case 'z': // Illumination mode (IS-1200)
				{
					Bool mode = FALSE;
					printf( "\nEnter illumination mode (1 = night, 0 = day): " );
					scanf( "%d", &mode );

					const char *modeStr = (mode) ? "night" : "day";

					if ( ISD_SetIlluminationMode( currentTrackerH, mode ) )
					{
						printf( "\nIllumination mode set to %s\n", modeStr );
					}
					else
					{
						printf( "\nFailed to set illumination mode to %s\n", modeStr );
					}
					break;
				}
				case 'y': // Exposure (IS-1200)
				{
					int exposure = 0;
					printf( "\nEnter exposure value (microseconds): " );
					scanf( "%d", &exposure );

					if ( exposure < 10 || exposure > 10000 )
					{
						printf( "\nInvalid exposure value. Valid range = [10, 10000]\n" );
					}
					else if ( ISD_SetExposure(currentTrackerH, exposure ) )
					{
						printf("\nExposure set to %d.\n", exposure);
					}
					else
				...
				default:
					...
					printf( "z -- Set illumination mode. (IS-1200 only)\n" );
					printf( "y -- Set exposure value.    (IS-1200 only)\n" );
			...
			if( currentTrackerH > 0 )
			{
				...
				if(logType == 1 && fpStation)
				{
					...
					while(data.Station[selectedStation-1].NewData == TRUE)
					{
						if(numRecordsSkipped[currentTrackerH-1][selectedStation-1] < numRecordsToSkip)
						{
							numRecordsSkipped[currentTrackerH-1][selectedStation-1]++;
						}
						else
						{
							numRecordsSkipped[currentTrackerH-1][selectedStation-1] = 0;
							logData(Trackers,&data.Station[selectedStation-1],trackerIdx,
							    selectedStation,fpStation,&Stations[selectedTracker-1][selectedStation-1],StationsHwInfo);
						}
						ISD_GetTrackingData( currentTrackerH, &data );
					}
				}
							...
							else
							{
								...
								if(validStation[trackerIdx][j])
									logData(Trackers,&data.Station[j],trackerIdx,
											j+1,fpStations,&Stations[selectedTracker-1][selectedStation-1],StationsHwInfo);
							}
						}
						ISD_GetTrackingData( currentTrackerH, &data );
					}
				...
				else if(logType == 3 && fpAll)
				{
					for(i=0; i < numOpenTrackers; i++)
					{
						...
						while(alternateTrackerData.Station[selectedStation-1].NewData == TRUE)
						{
							for(j=0; j < ISD_MAX_STATIONS; j++)
							{
								...
								else
								{
									...
									if(validStation[i][j])
										logData(Trackers,&alternateTrackerData.Station[j],i,
												j+1,fpAll,&Stations[selectedTracker-1][selectedStation-1],StationsHwInfo);
								}
							}
							ISD_GetTrackingData( Trackers[i], &alternateTrackerData );
						}
			...
			if( ISD_GetTime() - lastTime > 0.05f )
			{
				...
				if( currentTrackerH > 0 )
				{
					showStationData( currentTrackerH, &TrackerInfo,
									 &Stations[selectedTracker-1][selectedStation-1], &data.Station[selectedStation-1],
									 &StationsHwInfo[currentTrackerH-1][selectedStation-1],
									 showTemp);
				}
			}
#ifdef _WIN32
