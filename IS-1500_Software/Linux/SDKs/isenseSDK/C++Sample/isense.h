...
//						the IS-900, IS-1200, IS-1500, and NavChip.  Older sensors including the IS-300,
...
typedef enum
{
    ...
    ISD_PRECISION_SERIES,   // InertiaCubes, NavChip, IS-300, IS-600, IS-900, IS-1200, and IS-1500
...
typedef struct
{
    ...
    DWORD  TrackerType;    // IS Precision series or InterTrax. 
                           // TrackerType can be: 
                           // ISD_PRECISION_SERIES for InertiaCube, NavChip, IS-300, IS-600, IS-900, IS-1200, and IS-1500 model trackers, 
...
typedef struct
{
    ...
    Bool    State;              // TRUE if ON, FALSE if OFF. For IS-1500, setting to FALSE will reset streaming.

    Bool    Compass;            // Set to 0 or 2 for OFF or ON. Setting 1 is for special use cases
    ...
    BYTE    GetAHRSData;		// Obtain AHRS data from sensor instead of calculating it from sensor
    ...
    Bool    GetAuxInputs;     
    Bool    GetCovarianceData;  // TRUE to get covariances on IS-900 and IS-1200.
...
typedef struct
{
	...
	float   BatteryLevel;       // Battery voltage, if available.  Upper range may be limited, e.g. max 6.0V 
								// reported for IC4.
	...
	BYTE    HardIronCal;			// 1 if Hard Iron Compass calibration exists and is being applied, 0 otherwise. 		
	BYTE    SoftIronCal;			// 1 if Soft Iron Compass calibration exists and is being applied, 0 otherwise. 
	BYTE    EnvironmentalCal;		// 1 if Environmental Compass calibration exists. 
									// See CompassCompensation field more information.
	...
	DWORD   OSTimeStampMicroSec;	// Fractional part of the OS Time Stamp in micro-seconds

	BYTE    CompassQuality;			// If Environmental Calibration exists this value contains 
									// the calculated quality of the compass measurement based on deviation 
									// from nominal dip angle and magnitude values.
									// Values are 0-100.

	INT8    TrackingState;			// Denotes state of tracker:
									// 0: No communication.
									// 1: Tracking with relative position.
									// 2: Tracking with point of reference for position.
									// 3: Lost.
	BYTE    bReserved3;
	BYTE    bReserved4;
	...
	float	Cbn[3][3];				// 3x3 Rotational Matrix

	float	RotSig[3];				// roll/pitch/yaw orientation sigma (rad). ISD_STATION_INFO_TYPE->GetCovarianceData must be TRUE for IS-900 and IS-1200. Always retrieved for sfAccess devices.
	float	PosSig[3];				// X/Y/Z position sigma (m) ISD_STATION_INFO_TYPE->GetCovarianceData must be TRUE for IS-900 and IS-1200. Always retrieved for sfAccess devices.
	float	VelSig[3];				// X/Y/Z velocity sigma (m/s) Always retrieved for sfAccess devices.

	float	Temperature;			// Station temperature in degrees C (3DOF sensors only)
...
typedef struct
{
    ...
    float   CovarianceOrientation[3];		 // Available only for IS-900 and IS-1200. ISD_STATION_INFO_TYPE->GetCovarianceData must be TRUE
    float   CovariancePosition[3];			 // Available only for IS-900 and IS-1200. ISD_STATION_INFO_TYPE->GetCovarianceData must be TRUE
...
typedef struct
{
    ...
    struct
    {
        ...
        Bool    AHRS;		  // Supports AHRS data output (onboard processing of yaw/pitch/roll)
        Bool    bReserved3;     
        Bool    bReserved4;
...
typedef struct
{
	...
	struct
	{
		UINT timestamp;   // Minimum Recommended timestamp (HH:MM:SS from GPS converted to sec)
...
typedef struct
{
    ...
    char    DescVersion[20];   // Station Descriptor version; null terminated if less than 20 characters

    float   FirmwareRev;       // Station firmware revision
    DWORD   SerialNum;         // Legacy station serial number (digits only); deprecated; use FullSerialNum for new code
    ...
    char    FullSerialNum[17];	// Full serial number of device (null-terminated character array).
								// Use this for new code; the left-most numeric portion of the
								// serial number, if any, will still be written into SerialNum
	char    cReserved1[111];    
    char    cReserved2[128];    
    char    cReserved3[128];    
    char    cReserved4[128];    
...
// When used with IS Precision Series (IS-300, IS-600, IS-900, IS-1200, IS-1500) tracking devices 
...
DLLEXPORT Bool DLLENTRY ISD_SetIlluminationMode(ISD_TRACKER_HANDLE handle, Bool night);

DLLEXPORT Bool DLLENTRY ISD_SetExposure(ISD_TRACKER_HANDLE handle, int exposure);

// Returns size of image buffer (bytes). Only for IS-1500
DLLEXPORT DWORD DLLENTRY ISD_GetImageBufSize(ISD_TRACKER_HANDLE handle);

// Retrieves last received image. Only for IS-1500
// out: progress - 0=waiting, 100=done, 1-99=receiving
// out: format - image format. See ISD_IMAGE_FORMAT_TYPE
// out: sid - sequence id
// out: buf - image buffer
// in:  bufsize - size of image buffer
DLLEXPORT Bool DLLENTRY ISD_GetImage(ISD_TRACKER_HANDLE handle, int *progress, DWORD *format, DWORD *sid,
	BYTE buf[], int bufsize);

// Reports image info given format (outputs ignored if null). Only for IS-1500
// in:  format - image format. See ISD_IMAGE_FORMAT_TYPE
// out: size - image size in bytes
// out: w - image width in pixels
// out: h - image height in pixels
// out: bpp - bytes per pixel
DLLEXPORT Bool DLLENTRY ISD_GetImageInfo(ISD_TRACKER_HANDLE handle, DWORD format,
	int *size, int *width, int *height, int *bpp);

// Enables/disables image transfer. Only for IS-1500
DLLEXPORT Bool DLLENTRY ISD_SetImagesEnabled(ISD_TRACKER_HANDLE handle, Bool enabled);

// Retrieve GPS Data from sfHub. Only for IS-1500
DLLEXPORT Bool DLLENTRY ISD_GetGpsData(ISD_TRACKER_HANDLE handle, Bool *valid, ISD_GPS_DATA_TYPE *data);

// Send GPS data to sfHub. Only for IS-1500
DLLEXPORT Bool DLLENTRY ISD_SetGpsData(ISD_TRACKER_HANDLE handle, ISD_GPS_DATA_TYPE *data);
