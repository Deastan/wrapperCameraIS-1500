...
//////////////////////////////////////////////////////////////////////////////
//
//      File Name:      ISenseLib.cs
//      Description:    C# interface to InterSense DLL
//      Created:        2008-07-01
//      Updated:        2017-05-25
//      Author:         Rand Kmiec
//
...
//
//      Comments:       This file is a C# analog to the isense.h and isense.c files; it allows
//                      the InterSense DLL to be called from C#.  Please see the isense.h file for
...
namespace ISense
{
	public class ISenseLib
	{
		/*****************************************/
		/*                                       */
		/*   Enumerations from InterSense DLL    */
		/*                                       */
		/*****************************************/
		...
		public enum ISD_SYSTEM_TYPE
		{
			...
			ISD_PRECISION_SERIES,   // InertiaCube2, IS-300, IS-600, IS-900, IS-1200 and IS-1500
		...
		public enum ISD_IMAGE_FORMAT_TYPE
		{
			ISD_VGA_GRAY_8 = 10,
			ISD_VGA_GRAY_16,
			ISD_VGA_YUYV_8,
		...
		public const string ISLIB_VERSION = "4.2715";
		...
		public struct ISD_TRACKER_INFO_TYPE
		{
			...
			// TrackerType can be: 
			// ISD_PRECISION_SERIES for IS-300, IS-600, IS-900, IS-1200 and IS-1500 model trackers, 
		...
		public struct ISD_STATION_INFO_TYPE
		{
			...
			public bool State;          // TRUE if ON, FALSE if OFF. For IS-1500, setting to FALSE will reset streaming.
			...
			public int Prediction;     // -50 to 50 ms 
			...
			public bool GetCovarianceData;  // TRUE to get covariances on IS-900 and IS-1200.
		...
		public struct ISD_STATION_DATA_TYPE
		{
			...
			public sbyte TrackingState;         // Denotes state of tracker:
												// 0: No communication.
			...
			public float[] Cbn;       //Rotation Matrix. unfortunately, cannot marshal 2d array
			
			[MarshalAs(UnmanagedType.ByValArray, SizeConst = 3)]
			public float[] RotSig;    // roll/pitch/yaw orientation sigma (rad). ISD_STATION_INFO_TYPE->GetCovarianceData must be TRUE for IS-900 and IS-1200. Always retrieved for sfAccess devices.

			[MarshalAs(UnmanagedType.ByValArray, SizeConst = 3)]
			public float[] PosSig;    // X/Y/Z position sigma (m) ISD_STATION_INFO_TYPE->GetCovarianceData must be TRUE for IS-900 and IS-1200. Always retrieved for sfAccess devices.

			[MarshalAs(UnmanagedType.ByValArray, SizeConst = 3)]
			public float[] VelSig;    // X/Y/Z velocity sigma (m/s) Always retrieved for sfAccess devices.
		...
		public struct ISD_CAMERA_ENCODER_DATA_TYPE
		{
			...
			[MarshalAs(UnmanagedType.ByValArray, SizeConst = 3)]
			public float[] CovarianceOrientation;   // Available only for IS-900 and IS-1200. ISD_STATION_INFO_TYPE->GetCovarianceData must be TRUE
			
			[MarshalAs(UnmanagedType.ByValArray, SizeConst = 3)]
			public float[] CovariancePosition;		// Available only for IS-900 and IS-1200. ISD_STATION_INFO_TYPE->GetCovarianceData must be TRUE
		...
		public enum ISD_AUX_SYSTEM_TYPE
		{
		...
		// GPS Data
		[StructLayout(LayoutKind.Sequential, Pack = 4)]
		public struct ISD_GPS_DATA_TYPE
		{
			public struct GGA
			{
				public byte format; 		// GPS sentence format identifier
				public uint timestamp;		// GGA timestamp (HH:MM:SS from GPS converted to sec)
				public double latitude;		// GGA latitude
				public double longitude;	// GGA longitude
				public float altitude;		// Meters
				public UInt16 nSatellites; 	// Satellite count
				public byte quality;		// GPS fix quality. 1 = GPS fix
				public float geoIdHeight;	// Height of sea level (meters)
				public float hdop;			// Horizontal dilution of precision
			};

			public struct RMC
			{
				public uint timestamp;		// Minimum recommended timestamp (HH:MM:SS from GPS converted to sec)
				public double latitude;		// Minimum recommended latitude
				public double longitude;	// Minimum recommended longitude
				public float speed;			// m/s
				public float bearing;		// degrees
				public float magVar;		// Variation from magnetic north (degrees)
				public bool dataActive;		// Is GPS data good?
			};
		};
		...
		// this release). 
		// ----------------------------------------------------------------------------
		[DllImport("isense", CallingConvention = CallingConvention.Cdecl)]
		...
		[DllImport("isense", CallingConvention = CallingConvention.Cdecl)]
		public static extern bool ISD_SetIlluminationMode(int handle, bool night);
		
		[DllImport("isense", CallingConvention = CallingConvention.Cdecl)]
		public static extern bool ISD_SetExposure(int handle, int exposure);

		// Returns size of image buffer (bytes)
		[DllImport("isense", CallingConvention = CallingConvention.Cdecl)]
		public static extern uint ISD_GetImageBufSize(int handle);
		...
		[DllImport("isense", CallingConvention = CallingConvention.Cdecl)]
		public static extern bool ISD_GetImage(int handle, IntPtr progress, IntPtr format, IntPtr sid, Byte[] buf, uint bufsize);
		...
		[DllImport("isense", CallingConvention = CallingConvention.Cdecl)]
		public static extern bool ISD_GetImageInfo(int handle, int format, IntPtr size, IntPtr width, IntPtr height, IntPtr bpp);

		// Enables/disables image transfer
		[DllImport("isense", CallingConvention = CallingConvention.Cdecl)]
		public static extern bool ISD_SetImagesEnabled(int handle, bool enabled);

		// Retrieve GPS data from sfHub
		[DllImport("isense", CallingConvention = CallingConvention.Cdecl)]
		public static extern bool ISD_GetGpsData(int handle, ref Boolean valid, ref ISD_GPS_DATA_TYPE data);

		// Send GPS data to sfHub
		[DllImport("isense", CallingConvention = CallingConvention.Cdecl)]
		public static extern bool ISD_SetGpsData(int handle, ref ISD_GPS_DATA_TYPE data);
	}
}
